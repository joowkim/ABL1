...
3. <del> Correct short indel errors using [Pollux](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-014-0435-6)
4. Align reads to the reference genome (exon 4 to 10 of ABL1)
...
7. Filter out vcf files using cutoff from vcf files generated by freebayes.
   - Includes only sites with Quality value above this threshold, **10** in our case - Quality in vcf
   - Exclude all genotypes with a quality below the threshold specified, **10** in our case - GQ in vcf

**Terms**
- Quality: Phred-scaled quality score for the assertion made in ALT. i.e. −10log10 prob(call in ALT is
...
- GQ: Genotype quality. conditional genotype quality, encoded as a phred quality −10log10 p(genotype call is wrong, conditioned
on the site’s being variant) 
